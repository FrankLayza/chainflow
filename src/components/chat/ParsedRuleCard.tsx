...
export const ParsedRuleCard: React.FC<ParsedRuleCardProps> = ({
  ...
  return (
    <div className="w-full rounded-lg bg-smoke-charcoal border border-faint-linen/20 p-4">
      ...
      <p className="text-xs text-bone-gray font-mono mb-4 italic">
        Executes on-chain via KeeperHub Direct Execution API upon confirmation.
      </p>

      <div className="grid grid-cols-2 gap-px bg-iron-veil rounded-md overflow-hidden mb-4">
        <div className="bg-smoke-charcoal p-3">
          <span className="text-caption-tracked uppercase tracking-wider text-bone-gray font-mono block mb-0.5">Trigger</span>
          <span className="text-sm font-mono text-gold-leaf">{rule.ruleType}</span>
        </div>
        <div className="bg-smoke-charcoal p-3">
          <span className="text-caption-tracked uppercase tracking-wider text-bone-gray font-mono block mb-0.5">Threshold</span>
          <span className="text-sm font-mono text-warm-off-white">{rule.parameters.thresholdAmount || 'Immediate'} {rule.ruleType === 'PRICE_BELOW' ? 'USD' : 'ETH'}</span>
        </div>
        <div className="bg-smoke-charcoal p-3">
          <span className="text-caption-tracked uppercase tracking-wider text-bone-gray font-mono block mb-0.5">Action</span>
          <span className="text-sm font-mono text-muted-cobalt">Transfer {rule.parameters.transferAmount} ETH</span>
        </div>
        <div className="bg-smoke-charcoal p-3">
          <span className="text-caption-tracked uppercase tracking-wider text-bone-gray font-mono block mb-0.5">Recipient</span>
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-warm-off-white text-deep-ember text-sm font-medium transition-all duration-150 hover:bg-pale-stone active:scale-95 disabled:opacity-40 cursor-pointer"
        >
